...
pub async fn download(crate_name: &str) -> Result<String, Error> {
    let client = default_client()?;
    let content = download_with_client(crate_name, &client).await?;
    Ok(content)
}

/// Downloads the Cargo.toml content of the specified crate as a String using a [crates_io_api::AsyncClient].
pub async fn download_with_client(crate_name: &str, client: &CratesIoClient) -> Result<String, Error> {
    let version = latest_version(crate_name, client).await?;
...
async fn latest_version(crate_name: &str, client: &CratesIoClient) -> Result<crates_io_api::Version, Error> {
    let _crate = client.get_crate(crate_name).await?;
...
mod tests {
    ...
    async fn download_tokio() {
        let content = download("tokio").await.unwrap();
        assert_ne!(content, "");
    }
}